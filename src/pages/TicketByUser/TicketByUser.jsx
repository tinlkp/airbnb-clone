...
export default function TicketByUser() {
  ...
  const fectchRoomTicket = async () => {
    if (stateUser.userInfo) {
      const roomTicketData = await roomService.fetchRoomTicketApi(stateUser.userInfo.id);
      const roomTickets = roomTicketData.data;
      console.log(roomTickets)
      const roomDetails = [];
      for (const room of roomTickets) {
        const roomTicketDetail = await roomService.fetchRoomDetailApi(room.ma_phong);
        roomDetails.push(roomTicketDetail.data);
        console.log(roomTicketDetail.data)
      }
  ...
  const totalCost = roomTicket.reduce((acc, ticket, index) => {
    ...
    const roomCost = roomTicketDetail[index].gia_tien * numberOfDays;
    return acc + roomCost;
  }, 0);
            ...
            <div className='flex flex-col mt-4 md:pr-5 lg:w-2/3'>
              {roomTicket.map((ticket, index) => (
                <div key={ticket.id} className='border mb-4 rounded-lg hover:shadow-xl duration-200 flex cursor-pointer'>
                  <div className="w-1/4">
                    <img src={roomTicketDetail[index].hinh_anh} alt={roomTicketDetail[index].ten_phong} className="h-full w-full object-cover rounded-l-lg" />
                  </div>
                  <div className='w-3/4 px-4 py-2'>
                    <div className='flex justify-between items-center md:border-b pb-2'>
                      <div className="text-center md:text-left">
                        <h3 className="font-semibold text-xl md:text-xl uppercase">{roomTicketDetail[index].ten_phong}</h3>
                      ...
                      <div>
                        <span className="font-semibold text-sm md:text-xl">${roomTicketDetail[index].gia_tien}
                    ...
                    <div className='hidden md:block'>
                      ...
                      <div className='flex items-center'>
                        <div className='flex items-center'>
                          {roomTicketDetail[index].may_giat && (
                            <div>
                              <KitchenIcon className='inline text-gray-500 ml-2' />
                            </div>
                          )}
                          {roomTicketDetail[index].ban_la && (
                            <div>
                              <IronIcon className='inline text-gray-500 ml-2' />
                            </div>
                          )}
                          {roomTicketDetail[index].dieu_hoa && (
                            <div>
