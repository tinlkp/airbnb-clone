...
export default function RoomDetails() {
  ...
  const fetchRoomDetail = async () => {
    ...
    const ma_vi_tri = roomDetailResult.data.ma_vi_tri;
    if (ma_vi_tri > 0) {
      const roomLocationResult = await roomService.fetchRoomLocationDetailApi(
        ma_vi_tri
      );
      setRoomDetail(roomDetailResult.data);
      dispatch(setRoomInfoAction(roomDetailResult.data));
      setRoomDetailLocation(roomLocationResult.data);
    } else {
      setRoomDetail(roomDetailResult.data);
      dispatch(setRoomInfoAction(roomDetailResult.data));
    }
    const motaArray = roomDetailResult.data.mo_ta.split(regex);
          ...
          <div>
            ...
            <div className="flex flex-wrap justify-between items-center">
              <div className="flex items-center">
                ...
                <span className="underline text-sm font-normal tracking-widest mx-2">
                  <span className="mr-2">
                    {roomDetailLocation?.ten_vi_tri}{" "}
                  </span>
                  <span className="mr-2">
                    {roomDetailLocation?.tinh_thanh}{" "}
                  </span>
                  <span>{roomDetailLocation?.quoc_gia}</span>
                </span>
              </div>
              ...
              <div className="flex flex-wrap justify-between items-center pb-5 border-b">
                <div>
                  ...
                  <div className="flex flex-wrap items-center">
                    ...
                    {roomDetail?.phongTam > 0 && roomDetail?.phong_tam <= 8 && (
                      <span className="flex mr-2 border-r pr-2">
                        {[...Array(roomDetail.phong_tam)].map((_, index) => (
                          <ShowerIcon
                            fontSize="small"
                            key={index}
                            className="inline text-gray-500"
