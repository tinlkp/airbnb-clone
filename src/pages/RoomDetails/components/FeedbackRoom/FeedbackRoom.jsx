...
const CommentSchema = Yup.object().shape({
  noi_dung: Yup.string().required('Nội dung bình luận không được trống'),
  sao_binh_luan: Yup.number()
...
export default function FeedbackRoom(props) {
  ...
  const initialValues = {
    noi_dung: '',
    sao_binh_luan: rating,
    ngay_binh_luan: moment().format('ddd, DD MMM YYYY HH:mm:ss [GMT]'),
    ma_phong: props.feedBack.id,
    ma_nguoi_binh_luan: userState.userInfo ? userState.userInfo.user.id : "",
  };
  ...
  const onSubmitComment = async (values, { resetForm }) => {
    const dataSubmit = { ...values, sao_binh_luan: rating };
      ...
      return feedBackRoom.map((element) => (
        <div key={element.id} className="mb-5 p-4 bg-white rounded-lg shadow-md">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h4 className="font-bold text-lg text-gray-900">{element.ma_nguoi_binh_luan}</h4>
              <span className="text-sm text-gray-500">{element.ngay_binh_luan}</span>
            </div>
          </div>
          ...
          <div className="flex items-center">
            {renderStarRating(Number(element.sao_binh_luan))}
          </div>
        </div>
      ));
    } else {
      return feedBackRoom.slice(0, 4).map((element) => (
        <div key={element.id} className="mb-5 p-4 bg-white rounded-lg shadow-md">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h4 className="font-bold text-lg text-gray-900">{element.ma_nguoi_binh_luan}</h4>
              <span className="text-sm text-gray-500">{formatDate(element.ngay_binh_luan)}</span>
            </div>
          </div>
          <div className="text-gray-800 tracking-wider mt-4">
            <p className="font-bold text-sm text-gray-800">{element.noi_dung}</p>
          </div>
          <div className="flex items-center">
            {renderStarRating(Number(element.sao_binh_luan))}
          </div>
